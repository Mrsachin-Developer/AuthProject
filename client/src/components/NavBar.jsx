  ...
  const sendVerificationOtp = async () => {
    try {
      ...
      if (data.success) {
        ...
        toast.success(data.message); // Fixed here: removed quotes
      } else {
      ...
      {userData ? (
        <div className="w-8 h-8 flex justify-center items-center rounded-full bg-black text-white relative group cursor-pointer">
          ...
          <div className="absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-10">
            <ul className="list-none m-0 p-2 w-40 bg-gray-200 text-sm ">
              ...
              <li
                onClick={logout}
                className="py-1 px-2 hover:bg-gray-300 cursor-pointer pr-10"
              >
                Logout
              </li>
            </ul>
